...
begin 
 assign(it,'C:\OAIP\14\input2.txt');  {обеспечивает связь файловой переменной программы с реальным файлом на диске. Первым аргументом указывается переменная, вторым – адресное имя файла.}
 ...
 close(it); 
 assign(out,'C:\OAIP\14\output2.txt'); 
 rewrite(out); 
 reset(it); 
 var max:=0; 
 while not eof(it) do 
